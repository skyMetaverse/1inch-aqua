...
若 `dock` 已确认而 `ship` 因 RPC、余额、授权或合约状态变化失败，Bot 保留同一份已冻结钱包快照的计划并在下一轮优先恢复，不会生成第二个策略 hash 或悄悄修改投入金额。冻结后新增的钱包资产不会自动并入该次 ship；余额减少则阻止该计划执行。运行前应阅读 [Aqua自动再平衡Bot开发设计.md](/Users/syskey/git/1inch-aqua/docs/Aqua自动再平衡Bot开发设计.md)。
