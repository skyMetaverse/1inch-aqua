...
function logicalKey(strategy: ApiStrategy): string { return `${strategy.chainId}:${strategy.maker.toLowerCase()}:${strategy.app.toLowerCase()}:${[strategy.tokens[0].address, strategy.tokens[1].address].map((value) => value.toLowerCase()).sort().join(":")}`; }
...
async function executeShip(parameters: { plan: PersistedPlan; state: StateDocument; config: RebalanceConfig; client: ReturnType<typeof createPublicClient>; registry: Address; account: PrivateKeyAccount; chain: Chain; chainId: number; rpcUrl: string; logger: Logger }): Promise<PersistedPlan> {
  ...
  if (plan.stage === "SHIP_SENT") {
    ...
    plan = { ...plan, stage: "ACTIVE_LATEST", updatedAt: Date.now() }; updatePlan(parameters.state, plan, parameters.config); parameters.state.observations[plan.logicalPositionKey] = { strategyHash: built.strategyHash, breachCount: 0, lastShipAt: Date.now() }; saveRebalanceState(parameters.config.runtime.stateFile, parameters.state); parameters.logger.info(`恢复 ship 成功：strategyHash=${built.strategyHash}`); return plan;
  }
  ...
  plan = { ...plan, stage: "ACTIVE_LATEST", updatedAt: Date.now() }; updatePlan(parameters.state, plan, parameters.config); parameters.state.observations[plan.logicalPositionKey] = { strategyHash: built.strategyHash, breachCount: 0, lastShipAt: Date.now() }; saveRebalanceState(parameters.config.runtime.stateFile, parameters.state); parameters.logger.info(`自动重挂完成：旧=${plan.sourceStrategyHash}，新=${built.strategyHash}`); return plan;
}
...
/** 每轮处理 API 返回的完整仓位集合；同 pair 多策略或未支持策略均跳过，不猜测其含义。 */
async function processSnapshot(parameters: { strategies: ApiStrategy[]; config: RebalanceConfig; state: StateDocument; account: PrivateKeyAccount; app: Address; logger: Logger; execute: (plan: PersistedPlan) => Promise<void> }): Promise<void> {
  const grouped = new Map<string, ApiStrategy[]>(); for (const strategy of parameters.strategies) { const key = logicalKey(strategy); grouped.set(key, [...(grouped.get(key) ?? []), strategy]); }
  const supported: ApiStrategy[] = [];
  for (const [key, group] of grouped) { if (group.length !== 1) { parameters.logger.info(`跳过逻辑仓位=${key}：同一 pair 存在 ${group.length} 条 active 策略`); continue; } const strategy = group[0]; if (!strategy || strategy.maker.toLowerCase() !== parameters.account.address.toLowerCase() || strategy.chainId !== parameters.config.chainId || strategy.app.toLowerCase() !== parameters.app.toLowerCase() || strategy.classification.type !== "concentrated" || strategy.classification.state !== "active") { parameters.logger.info(`跳过 strategyHash=${strategy?.strategyHash ?? "unknown"}：maker、chain、app 或策略类型不受支持`); continue; } supported.push(strategy); }
  const markets = await getPairMarkets(parameters.config.chainId, supported.map((strategy) => [strategy.tokens[0].address, strategy.tokens[1].address]));
  for (const [index, strategy] of supported.entries()) {
