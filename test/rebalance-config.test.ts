/**
 * 自动再平衡配置的回归测试。
 * 核心功能：验证 5 bp 默认配置可解析，并拒绝已移除的成交量门槛、未知字段和不安全比例阈值。
...
const validConfig = {
  ...
  market: { maxPairPriceDeviationPercent: "0.20%", minimumPairSwaps: 1 },
...
test("拒绝已移除的成交量门槛、未知字段和超过 100% 的转双边比例", () => {
  expect(() => validateRebalanceConfig({ ...validConfig, market: { ...validConfig.market, minimumPairVolumeUsd: "1000" } })).toThrow("不支持字段");
