...
import { buildMaximumApprovalSteps, ETHEREUM_UNI, MAX_UINT96, ETHEREUM_USDT, MAX_UINT256 } from "../src/infra/erc20.ts";
...
test("标准 ERC20 已是 uint256 最大授权时不生成 approve", () => {
  expect(buildMaximumApprovalSteps(1, oneInch, MAX_UINT256, registry)).toEqual([]);
});

test("Ethereum 主网 UNI 的 uint96 最大授权不重复 approve", () => {
  expect(buildMaximumApprovalSteps(1, ETHEREUM_UNI, MAX_UINT96, registry)).toEqual([]);
});

test("Ethereum 主网 UNI 首次授权使用 uint96 最大值", () => {
  const steps = buildMaximumApprovalSteps(1, ETHEREUM_UNI, 0n, registry);
  expect(steps).toHaveLength(1);
  expect(steps[0]?.amount).toBe(MAX_UINT96);
  expect(steps[0]?.reason).toBe("设置 UNI uint96 最大授权");
});
