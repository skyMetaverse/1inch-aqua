 ...
 * 核心功能：验证批量交易使用正确 selector，并拒绝跨合约或非零 value 的危险子调用。
 * 主要流程：构造确定性 dock/ship 风格 calldata -> 编码并解码 multicall -> 断言参数和安全边界。
 */
import { expect, test } from "bun:test";
import { decodeFunctionData } from "viem";
import { AQUA_MULTICALL_ABI, buildAquaMulticallTransaction } from "../src/aqua/multicall.ts";
...
test("Aqua multicall 拒绝不足两个、跨合约和非零 value 子调用", () => {
  expect(() => buildAquaMulticallTransaction(registry, [{ to: registry, data: "0x12345678", value: 0n }])).toThrow("至少需要两个");
