...
Bot 的目标是持续维持 `lp.add.jsonc` 中声明的 LP 配置仓位数量，同时监控当前 maker 钱包的全部受支持 Aqua 集中流动性仓位；已有策略报价不再贴近市场或单边仓位已形成接近等值的两侧资产时，按实际状态自动关闭旧仓位并创建新仓位。`lp.add.jsonc` 只定义缺失槽位的初始创建模板，不限制已有策略在成交后的单边/双边模式。目标是维持策略可被 resolver 路由成交的竞争力，不以手续费最大化为唯一目标。
...
10. 以 `positions[].id` 将配置模板关联到当前 strategyHash；活跃数少于配置槽位数时按缺失模板补建，两个及以上缺失模板通过一笔 `multicall([ship...])` 原子创建，并在 API 索引宽限期内避免重复 ship。
...
```text
positions[].id -> 当前 strategyHash
```

该关联只决定缺失时使用哪个初始 LP 模板。动态重挂时，`targetMode` 始终由 API 当前余额和市场状态决定；成功 ship 后只把槽位关联迁移到新 hash，绝不把策略模式写回配置模板。
