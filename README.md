...
Bot 查询 1inch 官方策略 API `status=open` 返回的当前 maker 仓位，并自动处理受支持的 `classification.type=concentrated`、`classification.state=active` 策略。`illiquidity` 同样作为 open 仓位显示在面板中，但其交易语义尚未由 SDK 或真实接口确认，Bot 会明确阻止自动交易。策略 API 的 `currentBalance` 用于判断当前资产状态，EMSH current 用于计算新的 5 bp 区间。Pair API 的 `volumeUsd` 仅记录为观察数据；自动重挂仍要求 Pair 最少 swaps 和 Pair/EMSH 价格交叉校验。它不提供 `--dry-run`：解密私钥后会持续运行，并在满足连续越界、冷却期和上述价格安全条件时自动执行 `dock -> ship`。
